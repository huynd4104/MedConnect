...
public class DoctorDashboardController {
    ...
    @PostMapping("/doctor-dashboard/reject")
    @Transactional
    public String rejectAppointment(@RequestParam("appointmentId") Integer appointmentId,
        ...
        try {
            ...
            // 2. Lấy thông tin bác sĩ đang đăng nhập
            String email = auth.getName();
            User currentUser = userRepository.findByEmail(email)
                    .orElseThrow(() -> new RuntimeException("Không tìm thấy tài khoản người dùng."));
            ...
            // 3. (Bảo mật) Kiểm tra xem lịch hẹn này có phải của bác sĩ không
            Appointment appointment = appointmentRepository.findById(appointmentId)
                    .orElseThrow(() -> new RuntimeException("Không tìm thấy lịch hẹn."));
            ...
            // 4. Gọi service để từ chối
            appointmentService.rejectAppointmentByDoctor(appointmentId, reason);
            redirectAttributes.addFlashAttribute("success", "Đã từ chối lịch hẹn thành công.");
